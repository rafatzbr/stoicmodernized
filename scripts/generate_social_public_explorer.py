...
<head>
  ...
  <style>
    ...
    *{{box-sizing:border-box}} body{{margin:0;background:radial-gradient(circle at 22px 22px,rgba(255,255,255,.055) 1px,transparent 1px),var(--bg);background-size:22px 22px;color:var(--text);font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;line-height:1.45}}
    ...
    .summary{{display:flex;gap:18px;flex-wrap:wrap;color:var(--muted);font-size:14px;margin-bottom:14px}} .summary b{{color:var(--text)}} .summary .mono{{overflow-wrap:anywhere;word-break:break-word}}
    .list{{border:1px solid var(--line);background:rgba(12,12,12,.8);min-width:0;overflow:hidden}} .row{{display:grid;grid-template-columns:minmax(0,1fr) 130px 210px 98px;gap:14px;align-items:center;padding:14px 16px;border-bottom:1px solid var(--line);text-decoration:none;min-width:0}} .row:last-child{{border-bottom:0}} .row:hover{{background:var(--panel2)}} .head{{color:var(--muted);font-size:12px;text-transform:uppercase;letter-spacing:.1em;background:#0d0d0d}} .sort{{appearance:none;background:transparent;border:0;color:inherit;padding:0;font:inherit;text-transform:inherit;letter-spacing:inherit;cursor:pointer;text-align:left}} .sort:hover,.sort:focus{{color:var(--text);outline:0}} .sort .arrow{{display:inline-block;min-width:1em;color:var(--accent)}} .name{{display:flex;align-items:center;gap:10px;min-width:0;max-width:100%;overflow:hidden}} .icon{{width:28px;height:28px;display:inline-flex;align-items:center;justify-content:center;border:1px solid var(--line);background:var(--panel);flex:0 0 auto}} .name-text{{min-width:0;max-width:100%;flex:1 1 auto;overflow:hidden}} .label{{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:100%}} .sub{{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:100%;color:var(--muted);font-size:12px;margin-top:2px}} .kind,.date,.size{{color:var(--muted);font-size:14px;min-width:0;overflow:hidden;text-overflow:ellipsis}} .empty{{padding:42px;text-align:center;color:var(--muted)}}
